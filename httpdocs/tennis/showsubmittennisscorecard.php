...
function add_scorecard_step1($db)
{
	...
	if ($db->Exists("SELECT * FROM tennisteams")) {
		$db->Query("SELECT * FROM tennisteams ORDER BY TeamAbbrev");
		for ($i=0; $i<$db->rows; $i++) {
			$db->GetRow($i);
...
function insert_scorecard_step1($db,$season,$week,$awayteam,$hometeam,$umpires,$toss_won_id,$result_won_id,$batting_first_id,$batting_second_id,$ground_id,$game_date,$result,$forfeit,$cancelled)
{
...
////////////////////////////////////////////////////////////////////////////////////////////
	...
	echo "	<option value=\"\">---------------</option>\n";
	if ($db->Exists("SELECT * FROM tennisplayers")) {
		$db->Query("SELECT * FROM tennisplayers WHERE PlayerTeam = $bat1stid ORDER BY PlayerLName");
		for ($i=0; $i<$db->rows; $i++) {
			$db->GetRow($i);
	...
	echo "	<option value=\"\">---------------</option>\n";
	if ($db->Exists("SELECT * FROM tennisplayers")) {
		$db->Query("SELECT * FROM tennisplayers WHERE PlayerTeam = $bat2ndid ORDER BY PlayerLName");
		for ($i=0; $i<$db->rows; $i++) {
			$db->GetRow($i);
			echo "<option value=\"" . $db->data[PlayerID] . "\">" . $db->data[PlayerLName] . ", " . $db->data[PlayerFName] . "</option>\n";
		}
...
// 24-June-2010 12:57am - Added mom to case 1
