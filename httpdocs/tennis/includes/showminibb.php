...
function show_mini_bb($db,$schedule,$id,$pr,$team,$date,$sv)
{
        ...
        if (!$db->Exists("SELECT * FROM board_posts")) {
 		...
 		echo "    <p>There are currently no posts in the database.</p>\n";
			...
			$db->Query("SELECT p.post_id, p.topic_id, p.forum_id, p.post_username, pt.post_subject, pu.user_id, pu.username FROM (board_posts p INNER JOIN board_posts_text pt ON p.post_id = pt.post_id) INNER JOIN board_users pu ON p.poster_id = pu.user_id WHERE p.forum_id = 8 ORDER BY p.post_id DESC LIMIT 10");
			for ($x=0; $x<$db->rows; $x++) {
				$db->GetRow($x);
