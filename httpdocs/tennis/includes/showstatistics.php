...
function show_statistics_byseason($db,$statistics,$id,$pr,$team,$week,$game_id)
{
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
        ...
        $db->Query("SELECT la.season, se.SeasonName FROM tennis_scorecard_batting_details la INNER JOIN seasons se ON la.season = se.SeasonID WHERE se.SeasonName NOT LIKE '%KO%' GROUP BY la.season ORDER BY se.SeasonName DESC");
        for ($x=0; $x<$db->rows; $x++) {
            $db->GetRow($x);
            $db->BagAndTag();
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
        ...
        echo "</ul>\n";             
        echo "<ul>\n";          
        echo "<li><a href=\"$PHP_SELF?option=byseason&statistics=$statistics&ccl_mode=7\">All-Round: 30 Runs & 3 Wickets</li>\n";
        echo "</ul>\n"; 
        echo "</td>\n";
        echo "</tr>\n";
        echo "</table><br>\n";
...
function show_statistics_team($db,$statistics,$id,$pr,$team,$week,$game_id)
{
    ...
    $subdb->QueryRow("SELECT b.notout, MAX(b.runs) AS HS FROM tennis_scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' GROUP BY b.notout ORDER BY HS DESC LIMIT 1");
    ...
    $subdb->QueryRow("SELECT COUNT(b.wickets) AS fourwickets FROM tennis_scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.wickets = 4");
...
function show_statistics_mostruns($db,$statistics,$sort,$sort2,$option,$team)
{
    ...
    if($option == "byseason")   $db->Query("SELECT g.season, n.SeasonName, t.TeamID, t.TeamAbbrev, COUNT( s.player_id ) AS Matches, SUM( s.runs ) AS Runs, SUM( s.notout ) AS Notouts, COUNT( s.player_id ) - SUM( s.notout ) AS Innings, SUM( s.runs ) / (COUNT( s.player_id ) - SUM( s.notout )) AS Average, s.player_id, p.PlayerID, LEFT(p.PlayerFName,1) AS PlayerInitial, p.PlayerFName, p.PlayerLName, p.PlayerLAbbrev FROM tennis_scorecard_batting_details s INNER JOIN tennisplayers p ON s.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON s.game_id = g.game_id INNER JOIN tennisteams t ON p.PlayerTeam = t.TeamID INNER JOIN seasons n ON g.season = n.SeasonID WHERE n.SeasonName LIKE '%{$statistics}%' GROUP BY s.player_id HAVING (SUM( s.runs )) >=0 ORDER BY $sort DESC, $sort2 DESC");
    if($option == "allcareer")  $db->Query("SELECT g.season, t.TeamID, t.TeamAbbrev, COUNT( s.player_id ) AS Matches, SUM( s.runs ) AS Runs, SUM( s.notout ) AS Notouts, COUNT( s.player_id ) - SUM( s.notout ) AS Innings, SUM( s.runs ) / (COUNT( s.player_id ) - SUM( s.notout )) AS Average, s.player_id, p.PlayerID, LEFT(p.PlayerFName,1) AS PlayerInitial, p.PlayerFName, p.PlayerLName, p.PlayerLAbbrev FROM tennis_scorecard_batting_details s INNER JOIN tennisplayers p ON s.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON s.game_id = g.game_id INNER JOIN tennisteams t ON p.PlayerTeam = t.TeamID GROUP BY s.player_id HAVING (SUM( s.runs )) >=100 ORDER BY $sort DESC, $sort2 DESC");
    if($option == "teamcareer") $db->Query("SELECT g.season, t.TeamID, t.TeamAbbrev, COUNT( s.player_id ) AS Matches, SUM( s.runs ) AS Runs, SUM( s.notout ) AS Notouts, COUNT( s.player_id ) - SUM( s.notout ) AS Innings, SUM( s.runs ) / (COUNT( s.player_id ) - SUM( s.notout )) AS Average, s.player_id, p.PlayerID, LEFT(p.PlayerFName,1) AS PlayerInitial, p.PlayerFName, p.PlayerLName, p.PlayerLAbbrev FROM tennis_scorecard_batting_details s INNER JOIN tennisplayers p ON s.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON s.game_id = g.game_id INNER JOIN tennisteams t ON p.PlayerTeam = t.TeamID WHERE p.PlayerTeam = $team GROUP BY s.player_id HAVING (COUNT( s.player_id )) >=3 ORDER BY $sort DESC, $sort2 DESC");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.runs) AS Hundred FROM tennis_scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND b.runs >= 100");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.runs) AS Hundred FROM tennis_scorecard_batting_details b WHERE b.player_id = $playerid  AND b.runs >= 100");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.runs) AS Hundred FROM tennis_scorecard_batting_details b WHERE b.player_id = $playerid  AND b.team=$team AND b.runs >= 100");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.runs) AS Fifty FROM tennis_scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND (b.runs BETWEEN 50 AND 99) ");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.runs) AS Fifty FROM tennis_scorecard_batting_details b WHERE b.player_id = $playerid  AND (b.runs BETWEEN 50 AND 99) ");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.runs) AS Fifty FROM tennis_scorecard_batting_details b WHERE b.player_id = $playerid  AND b.team=$team AND (b.runs BETWEEN 50 AND 99) ");      
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.assist) AS Caught FROM tennis_scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.assist = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 4");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.assist) AS Caught FROM tennis_scorecard_batting_details b WHERE b.assist = $playerid  AND b.how_out = 4");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.assist) AS Caught FROM tennis_scorecard_batting_details b WHERE b.assist = $playerid  AND b.opponent=$team AND b.how_out = 4");

      $scctc = $subdb->data[Caught];    

    // Get Caught and Bowleds

    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.bowler) AS CandB FROM tennis_scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.bowler = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 5");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.bowler) AS CandB FROM tennis_scorecard_batting_details b WHERE b.bowler = $playerid  AND b.how_out = 5");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.bowler) AS CandB FROM tennis_scorecard_batting_details b WHERE b.bowler = $playerid  AND b.opponent=$team AND b.how_out = 5");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.assist) AS Stumped FROM tennis_scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.assist = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 10");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.assist) AS Stumped FROM tennis_scorecard_batting_details b WHERE b.assist = $playerid  AND b.how_out = 10");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.assist) AS Stumped FROM tennis_scorecard_batting_details b WHERE b.assist = $playerid  AND b.opponent=$team AND b.how_out = 10");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT b.notout, MAX(b.runs) AS HS FROM tennis_scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' GROUP BY b.notout ORDER BY HS DESC LIMIT 1");
    if($option == "allcareer")  $subdb->QueryRow("SELECT b.notout, MAX(b.runs) AS HS FROM tennis_scorecard_batting_details b WHERE b.player_id = $playerid GROUP BY b.notout ORDER BY HS DESC LIMIT 1");
    if($option == "teamcareer") $subdb->QueryRow("SELECT b.notout, MAX(b.runs) AS HS FROM tennis_scorecard_batting_details b WHERE b.player_id = $playerid AND b.team=$team GROUP BY b.notout ORDER BY HS DESC LIMIT 1");       
...
function show_statistics_bestinnings($db,$statistics,$option,$team)
{
    ...
    if($option == "byseason")   echo "<b class=\"16px\">$statistics Batting - Highest Innings Scores</b><br>Including Knock-Outs. Qualification 20 runs.<br><br>\n";
    if($option == "allcareer")  echo "<b class=\"16px\">Career Batting - Highest Innings Scores</b><br>From <b>$d</b> to the present. Qualification 25 runs.<br><br>\n";
    ...
    if($option == "byseason")   $db->Query("SELECT g.season, n.SeasonName, g.game_id, g.game_date, g.awayteam, g.hometeam, r.GroundAbbrev AS Ground, a.TeamAbbrev AS AwayAbbrev, h.TeamAbbrev AS HomeAbbrev, b.player_id, b.runs AS Runs, b.notout, p.PlayerLName, p.PlayerFName, p.PlayerLAbbrev, LEFT(p.PlayerFName,1) AS PlayerInitial FROM tennis_scorecard_batting_details b INNER JOIN tennisplayers p ON b.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON b.game_id = g.game_id INNER JOIN tennisteams a ON g.awayteam = a.TeamID INNER JOIN tennisteams h ON g.hometeam = h.TeamID INNER JOIN grounds r ON g.ground_id = r.GroundID INNER JOIN seasons n ON g.season = n.SeasonID WHERE n.SeasonName LIKE '%{$statistics}%' AND Runs >= 20 ORDER BY Runs DESC");
    if($option == "allcareer")  $db->Query("SELECT g.season, g.game_id, g.game_date, g.awayteam, g.hometeam, r.GroundAbbrev AS Ground, a.TeamAbbrev AS AwayAbbrev, h.TeamAbbrev AS HomeAbbrev, b.player_id, b.runs AS Runs, b.notout, p.PlayerLName, p.PlayerFName, p.PlayerLAbbrev, LEFT(p.PlayerFName,1) AS PlayerInitial FROM tennis_scorecard_batting_details b INNER JOIN tennisplayers p ON b.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON b.game_id = g.game_id INNER JOIN tennisteams a ON g.awayteam = a.TeamID INNER JOIN tennisteams h ON g.hometeam = h.TeamID INNER JOIN grounds r ON g.ground_id = r.GroundID WHERE Runs >= 25 ORDER BY Runs DESC");
    if($option == "teamcareer") $db->Query("SELECT g.season, g.game_id, g.game_date, g.awayteam, g.hometeam, r.GroundAbbrev AS Ground, a.TeamAbbrev AS AwayAbbrev, h.TeamAbbrev AS HomeAbbrev, b.player_id, b.runs AS Runs, b.notout, p.PlayerLName, p.PlayerFName, p.PlayerLAbbrev, LEFT(p.PlayerFName,1) AS PlayerInitial FROM tennis_scorecard_batting_details b INNER JOIN tennisplayers p ON b.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON b.game_id = g.game_id INNER JOIN tennisteams a ON g.awayteam = a.TeamID INNER JOIN tennisteams h ON g.hometeam = h.TeamID INNER JOIN grounds r ON g.ground_id = r.GroundID WHERE p.PlayerTeam = $team AND Runs >= 35 ORDER BY Runs DESC");
...
function show_statistics_bowling($db,$statistics,$sort,$direction,$option,$team)
{
    ...
    if (!$db->Exists("SELECT g.season, n.SeasonName, COUNT( s.player_id ) AS Matches, s.player_id, p.PlayerID, LEFT(p.PlayerFName,1) AS PlayerInitial, p.PlayerFName, p.PlayerLName FROM tennis_scorecard_bowling_details s INNER JOIN tennisplayers p ON s.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON s.game_id = g.game_id INNER JOIN seasons n ON g.season = n.SeasonID GROUP BY s.player_id")) {
    ...
    if($option == "allcareer") {
      if($sort == "Wickets") echo "<b class=\"16px\">Career Bowling - Most Wickets</b><br>From <b>$d</b> to the present. Qualification 10 overs.<br><br>\n";
      if($sort == "Balls")   echo "<b class=\"16px\">Career Bowling - Workhorses</b><br>From <b>$d</b> to the present. Qualification 10 overs.<br><br>\n";
      if($sort == "Average") echo "<b class=\"16px\">Career Bowling - Best Averages</b><br>From <b>$d</b> to the present. Qualification 10 overs.<br><br>\n";
    }
    ...
    if($option == "byseason")   $db->Query("SELECT g.season, n.SeasonName, t.TeamID, t.TeamAbbrev, b.player_id, SUM(IF(INSTR(overs, '.'),((LEFT(overs, INSTR(overs, '.') - 1) * 6) + RIGHT(overs, INSTR(overs, '.') - 1)),(overs * 6))) AS Balls, SUM( b.maidens ) AS Maidens, SUM( b.runs ) AS BRuns, SUM( b.wickets ) AS Wickets, SUM( b.runs ) / SUM( b.wickets ) AS Average, p.PlayerLName, p.PlayerFName, p.PlayerLAbbrev, LEFT(p.PlayerFName,1) AS PlayerInitial FROM tennis_scorecard_bowling_details b INNER JOIN tennisplayers p ON b.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON b.game_id = g.game_id INNER JOIN tennisteams t ON p.PlayerTeam = t.TeamID INNER JOIN seasons n ON g.season = n.SeasonID WHERE n.SeasonName LIKE '%{$statistics}%' GROUP BY b.player_id HAVING Balls >=0 ORDER BY $sort $direction, Average ASC");
    if($option == "allcareer")  $db->Query("SELECT g.season, t.TeamID, t.TeamAbbrev, b.player_id, SUM(IF(INSTR(overs, '.'),((LEFT(overs, INSTR(overs, '.') - 1) * 6) + RIGHT(overs, INSTR(overs, '.') - 1)),(overs * 6))) AS Balls, SUM( b.maidens ) AS Maidens, SUM( b.runs ) AS BRuns, SUM( b.wickets ) AS Wickets, SUM( b.runs ) / SUM( b.wickets ) AS Average, p.PlayerLName, p.PlayerFName, p.PlayerLAbbrev, LEFT(p.PlayerFName,1) AS PlayerInitial FROM tennis_scorecard_bowling_details b INNER JOIN tennisplayers p ON b.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON b.game_id = g.game_id INNER JOIN tennisteams t ON p.PlayerTeam = t.TeamID GROUP BY b.player_id HAVING Balls >=60 ORDER BY $sort $direction, Average ASC");
    if($option == "teamcareer") $db->Query("SELECT g.season, t.TeamID, t.TeamAbbrev, b.player_id, SUM(IF(INSTR(overs, '.'),((LEFT(overs, INSTR(overs, '.') - 1) * 6) + RIGHT(overs, INSTR(overs, '.') - 1)),(overs * 6))) AS Balls, SUM( b.maidens ) AS Maidens, SUM( b.runs ) AS BRuns, SUM( b.wickets ) AS Wickets, SUM( b.runs ) / SUM( b.wickets ) AS Average, p.PlayerLName, p.PlayerFName, p.PlayerLAbbrev, LEFT(p.PlayerFName,1) AS PlayerInitial FROM tennis_scorecard_bowling_details b INNER JOIN tennisplayers p ON b.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON b.game_id = g.game_id INNER JOIN tennisteams t ON p.PlayerTeam = t.TeamID WHERE p.PlayerTeam = $team GROUP BY b.player_id ORDER BY $sort $direction, Average ASC");
    ...
    for ($r=0; $r<$db->rows; $r++) {
      ...
      if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.wickets) AS fourwickets FROM tennis_scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.wickets = 4");
      if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.wickets) AS fourwickets FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid AND b.wickets = 4");
      if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.wickets) AS fourwickets FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid AND b.team=$team AND b.wickets = 4");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.wickets) AS fivewickets FROM tennis_scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.wickets >= 5");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.wickets) AS fivewickets FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid AND b.wickets >= 5");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.wickets) AS fivewickets FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid AND b.team=$team AND b.wickets >= 5");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT b.player_id, b.wickets, b.runs FROM tennis_scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' ORDER BY b.wickets DESC, b.runs ASC LIMIT 1");
    if($option == "allcareer")  $subdb->QueryRow("SELECT b.player_id, b.wickets, b.runs FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid ORDER BY b.wickets DESC, b.runs ASC LIMIT 1");
    if($option == "teamcareer") $subdb->QueryRow("SELECT b.player_id, b.wickets, b.runs FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid ORDER BY b.wickets DESC, b.runs ASC LIMIT 1");
...
function show_statistics_bestbowling($db,$statistics,$option,$team)
{
    ...
    if($option == "byseason")   $db->Query("SELECT g.season, n.SeasonName, g.game_id, g.game_date, g.awayteam, g.hometeam, r.GroundAbbrev AS Ground, a.TeamAbbrev AS AwayAbbrev, h.TeamAbbrev AS HomeAbbrev, b.player_id, b.runs AS BRuns, b.wickets AS Wickets, p.PlayerLName, p.PlayerLAbbrev, p.PlayerFName, LEFT(p.PlayerFName,1) AS PlayerInitial FROM tennis_scorecard_bowling_details b INNER JOIN tennisplayers p ON b.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON b.game_id = g.game_id INNER JOIN tennisteams a ON g.awayteam = a.TeamID INNER JOIN tennisteams h ON g.hometeam = h.TeamID INNER JOIN grounds r ON g.ground_id = r.GroundID INNER JOIN seasons n ON g.season = n.SeasonID WHERE n.SeasonName LIKE '%{$statistics}%' AND Wickets >= 3 ORDER BY Wickets DESC, Runs ASC");
    if($option == "allcareer")  $db->Query("SELECT g.season, g.game_id, g.game_date, g.awayteam, g.hometeam, r.GroundAbbrev AS Ground, a.TeamAbbrev AS AwayAbbrev, h.TeamAbbrev AS HomeAbbrev, b.player_id, b.runs AS BRuns, b.wickets AS Wickets, p.PlayerLName, p.PlayerLAbbrev, p.PlayerFName, LEFT(p.PlayerFName,1) AS PlayerInitial FROM tennis_scorecard_bowling_details b INNER JOIN tennisplayers p ON b.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON b.game_id = g.game_id INNER JOIN tennisteams a ON g.awayteam = a.TeamID INNER JOIN tennisteams h ON g.hometeam = h.TeamID INNER JOIN grounds r ON g.ground_id = r.GroundID WHERE Wickets >= 4 ORDER BY Wickets DESC, Runs ASC");
    if($option == "teamcareer") $db->Query("SELECT g.season, g.game_id, g.game_date, g.awayteam, g.hometeam, r.GroundAbbrev AS Ground, a.TeamAbbrev AS AwayAbbrev, h.TeamAbbrev AS HomeAbbrev, b.player_id, b.runs AS BRuns, b.wickets AS Wickets, p.PlayerLName, p.PlayerLAbbrev, p.PlayerFName, LEFT(p.PlayerFName,1) AS PlayerInitial FROM tennis_scorecard_bowling_details b INNER JOIN tennisplayers p ON b.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON b.game_id = g.game_id INNER JOIN tennisteams a ON g.awayteam = a.TeamID INNER JOIN tennisteams h ON g.hometeam = h.TeamID INNER JOIN grounds r ON g.ground_id = r.GroundID WHERE p.PlayerTeam = $team AND Wickets >= 3 ORDER BY Wickets DESC, Runs ASC");
...
function show_statistics_allrounders($db,$statistics,$option,$team)
{
    ...
    if($option == "byseason")   echo "<b class=\"16px\">$statistics All-Round - 30 Runs & 3 Wickets</b><br>Including Knock-Outs<br><br>\n";
    if($option == "allcareer")  echo "<b class=\"16px\">Career All-Round - 50 Runs & 5 Wickets</b><br>From <b>$d</b> to the present.<br><br>\n";
    if($option == "teamcareer") echo "<b class=\"16px\">{$teams[$team]} Career All-Round - 100 Runs & 10 Wickets</b><br>From <b>$d</b> to the present.<br><br>\n";
    ...
    if($option == "byseason")   $db->Query("SELECT g.season, n.SeasonName, t.TeamID, t.TeamAbbrev, COUNT( s.player_id ) AS Matches, SUM( s.runs ) AS Runs, MAX( s.runs ) AS HS, s.player_id, p.PlayerID, LEFT(p.PlayerFName,1) AS PlayerInitial, p.PlayerFName, p.PlayerLAbbrev, p.PlayerLName FROM tennis_scorecard_batting_details s INNER JOIN tennisplayers p ON s.player_id = p.PlayerID INNER JOIN tennis_scorecard_game_details g ON s.game_id = g.game_id INNER JOIN tennisteams t ON p.PlayerTeam = t.TeamID INNER JOIN seasons n ON g.season = n.SeasonID WHERE n.SeasonName LIKE '%{$statistics}%' GROUP BY p.PlayerID ORDER BY p.PlayerLName");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.how_out) AS Notout FROM tennis_scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.how_out = 2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%'");
    if($option == "allcareer")   $subdb->QueryRow("SELECT COUNT(b.how_out) AS Notout FROM tennis_scorecard_batting_details b WHERE b.how_out = 2 AND b.player_id = $playerid");
    if($option == "teamcareer")   $subdb->QueryRow("SELECT COUNT(b.how_out) AS Notout FROM tennis_scorecard_batting_details b WHERE b.how_out = 2 AND b.player_id = $playerid AND b.team=$team");
    ...
    if($option == "byseason")   $subruns = htmlentities(stripslashes($subdb->QueryItem("SELECT SUM(b.runs) AS Runs FROM tennis_scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%'")));
    if($option == "allcareer")   $subruns = htmlentities(stripslashes($subdb->QueryItem("SELECT SUM(b.runs) AS Runs FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid")));
    if($option == "teamcareer")   $subruns = htmlentities(stripslashes($subdb->QueryItem("SELECT SUM(b.runs) AS Runs FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid AND b.team=$team")));
    ...
    if($option == "byseason")   $subwic = htmlentities(stripslashes($subdb->QueryItem("SELECT SUM(b.wickets) AS Wickets FROM tennis_scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%'")));
    if($option == "allcareer")   $subwic = htmlentities(stripslashes($subdb->QueryItem("SELECT SUM(b.wickets) AS Wickets FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid")));
    if($option == "teamcareer")   $subwic = htmlentities(stripslashes($subdb->QueryItem("SELECT SUM(b.wickets) AS Wickets FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid AND b.team=$team")));
    ...
    if($option == "byseason")   $exist1 = htmlentities(stripslashes($subdb->QueryItem("SELECT MAX(b.player_id) AS Player FROM tennis_scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%'")));
    if($option == "allcareer")   $exist1 = htmlentities(stripslashes($subdb->QueryItem("SELECT MAX(b.player_id) AS Player FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid")));
    if($option == "teamcareer")   $exist1 = htmlentities(stripslashes($subdb->QueryItem("SELECT MAX(b.player_id) AS Player FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid AND b.team=$team")));

    if($exist1 != "") {

    if($option == "byseason")   $subdb->QueryRow("SELECT b.wickets AS BWickets, b.runs AS BRuns FROM tennis_scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID WHERE b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' ORDER BY b.wickets DESC, b.runs ASC LIMIT 1");
    if($option == "allcareer")   $subdb->QueryRow("SELECT b.wickets AS BWickets, b.runs AS BRuns FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid ORDER BY b.wickets DESC, b.runs ASC LIMIT 1");
    if($option == "teamcareer")   $subdb->QueryRow("SELECT b.wickets AS BWickets, b.runs AS BRuns FROM tennis_scorecard_bowling_details b WHERE b.player_id = $playerid AND b.team=$team ORDER BY b.wickets DESC, b.runs ASC LIMIT 1");
    ...
    if($option == "byseason" && $scrun >= "30" && $scwic >= "3") {
    ...
    } else if($option == "allcareer" && $scrun >= "50" && $scwic >= "5") {
    ...
    } else if($option == "teamcareer" && $scrun >= "100" && $scwic >= "10") {
