...
function show_teams_listing($db,$s,$id,$pr)
{
    ...
    if ($db->Exists("SELECT * FROM tennisteams")) {
		$db->QueryRow("SELECT * FROM tennisteams ORDER BY TeamActive DESC, TeamName ASC"); // 7-Nov-2014 10:07pm
    // 2-Dec-2009
      // $db->QueryRow("SELECT * FROM tennisteams WHERE LeagueID = 1 ORDER BY TeamActive DESC, TeamName ASC");
...
echo "    <td width=60% align=\"left\"><a href=\"teams.php?teams=$id&ccl_mode=1\">$na</a> <font class=\"10px\">($ta)</a>\n";
...
//  echo "  <a href=\"/index.php\">Home</a> &raquo; <a href=\"/teamdetails.php\">Teams</a> &raquo; <font class=\"10px\">Team Page</font></p>\n";
    ...
    echo "<td width=\"100%\" align=\"left\">\n";
       ...
       $db->QueryRow("SELECT picture FROM tennisteams WHERE TeamID=$pr");  // 7-Nov-2014 10:10pm
    // $db->QueryRow("SELECT picture, teamlogo FROM tennisteams WHERE TeamID=$pr");
    $db->BagAndTag();
...
//    $teamlogo = $db->data[teamlogo];
	...
	if ($teamlogo != "" ) {
		...
		echo "<a href=\"http://www.coloradocricket.org/uploadphotos/teams/$teamlogo\" onClick=\"return enlarge('http://www.coloradocricket.org/uploadphotos/teams/$teamlogo',event)\"><img src=\"http://www.coloradocricket.org/uploadphotos/teams/$teamlogo\" width=$width align=\"right\" border=\"1\"></a>\n";
...
// commented this block as there is no tennischampions table - 7-Nov-2014 10:14pm
...
/* commented block 7-Nov-2014 10:30pm
    echo "<table width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\" bordercolor=\"#$co\" align=\"center\">\n";
    echo "  <tr>\n";
...
*/
    ...
    LEFT JOIN
      tennis_scorecard_batting_details sb
    ON
      pl.PlayerID = sb.player_id
    LEFT JOIN 
      tennis_scorecard_game_details sg
    ON 
    ...
    // output story
		...
		for ($s=0; $s<$db->rows; $s++) {
			...
			if( ($s % 5) == 0 && $s == 0) {
        		...
        		echo "    <td width=85 height=80 align=center valign=top><center><a  href=\"/players.php?players=$pi&ccl_mode=1\"><img alt='$ln, $fn' align=center width=50 height=67 border=1 src=\"http://www.coloradocricket.org/uploadphotos/players/$pc\"><br>$ln,<br>$fn</a></center></td>\n";
        ...
        echo "<tr class=\"trrow2\">\n";
        echo "    <td width=\"100%\" colspan=\"3\">No players at this time</td>\n";
        echo "  </tr>\n";
    ...
    echo "<a href=\"teams.php?teams=$id&letter=A&ccl_mode=2&status=$active_status\">A</a>\n";
...
//  include("rightteams.php");   7-Nov-2014 10:28pm
...
function show_alpha_listing($db,$s,$id,$pr,$letter)
{
    ...
    if ($db->Exists("SELECT pl.PlayerID, pl.PlayerFName, pl.PlayerLName, pl.PlayerEmail, pl.PlayerClub, pl.isactive FROM tennisplayers pl INNER JOIN tennisteams te ON pl.PlayerTeam = te.TeamID WHERE pl.PlayerLName LIKE '{$letter}%' AND pl.PlayerTeam = $pr ORDER BY pl.PlayerLName")) {
    $db->QueryRow("
    SELECT
...
function changeURL(mode, team) {
	...
	document.location.href = "http://tennis.coloradocricket.org/teams.php?teams=" + team + "&ccl_mode=" + mode + "&status=" + status;
}
