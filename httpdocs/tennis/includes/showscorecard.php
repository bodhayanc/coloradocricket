...
function show_schedule_listing($db,$schedule,$id,$pr,$team,$week,$game_id)
{
    ...
    if ($db->Exists("SELECT ga.season, se.SeasonName FROM tennis_scorecard_game_details ga INNER JOIN seasons se ON ga.season = se.SeasonID GROUP BY ga.season ORDER BY se.SeasonName DESC")) {
        $db->QueryRow("SELECT ga.season, se.SeasonName FROM tennis_scorecard_game_details ga INNER JOIN seasons se ON ga.season = se.SeasonID GROUP BY ga.season ORDER BY se.SeasonName DESC");
...
function show_schedule($db,$schedule,$id,$pr,$team,$week,$game_id)
{
		...
		//16-Apr-2017 - Bodha - Modified to list only the teams associated with the scorecards for that particular season
            	//$db->Query("SELECT * FROM tennisteams WHERE LeagueID = 1 OR LeagueID = 4 ORDER BY TeamName");
                $db->Query("SELECT t.* FROM tennisteams t, tennis_scorecard_game_details s WHERE (s.awayteam = t.TeamID OR s.hometeam = t.TeamID) AND s.season=$schedule GROUP BY t.TeamID ORDER BY TeamName");
                for ($i=0; $i<$db->rows; $i++) {
                        $db->GetRow($i);
        ...
        // List by team for scorecard
            	...
            	//16-Apr-2017 - Bodha - Modified to list only the teams associated with the scorecards for that particular season
            	//$db->QueryRow("SELECT * FROM tennisteams WHERE LeagueID = 1 OR LeagueID = 4 AND TeamActive=1 ORDER BY TeamName");
                $db->QueryRow("SELECT t.* FROM tennisteams t, tennis_scorecard_game_details s WHERE (s.awayteam = t.TeamID OR s.hometeam = t.TeamID) AND s.season=$schedule GROUP BY t.TeamID ORDER BY TeamName");
        ...
        if (!$db->Exists("SELECT * FROM tennis_scorecard_game_details WHERE season=$schedule AND isactive=0")) {
...
function show_schedule_game($db,$schedule,$id,$pr,$team,$week,$game_id)
{
    ...
    if($db->Exists("Select * from  tennis_scorecard_game_details where forfeit='1' and game_id=$game_id")) {
     $db->QueryRow("
	    SELECT
	      s.*,
	      o.SeasonName,
    ...
    LEFT JOIN
      tennisplayers p ON s.mom = p.PlayerID
    LEFT JOIN
      tennisplayers p2 ON s.mom2 = p2.PlayerID
    LEFT JOIN
      tennisplayers u1 ON s.umpire1 = u1.PlayerID
    LEFT JOIN
      tennisplayers u2 ON s.umpire2 = u2.PlayerID
...
$db->QueryRow("SELECT report FROM tennis_scorecard_game_details WHERE game_id=$game_id");
