...
function search_site($db,$search="")
{
    ...
    if ($search != "")
    {
        ...
        $db->Query("SELECT te.TeamID, te.TeamAbbrev, pl.* FROM players pl INNER JOIN teams te ON pl.PlayerTeam = te.TeamID WHERE $playercontains ORDER BY pl.PlayerLName");
            if ($db->rows)
            {
        ...
        $db->Query("SELECT * FROM news WHERE $newscontains ORDER BY id DESC");
            if ($db->rows)
            {
        ...
        $db->Query("SELECT * FROM news WHERE $featuredcontains ORDER BY id DESC");
            if ($db->rows)
            {
        ...
        // Search Documents Box

        $documentscontains = "article LIKE '%{$search}%' OR title LIKE '%{$search}%'";

        $db->Query("SELECT * FROM documents WHERE $documentscontains ORDER BY id DESC");
            if ($db->rows)
            {

            echo "<table width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\" bordercolor=\"$greenbdr\" align=\"center\">\n";
            echo "  <tr>\n";
            echo "    <td bgcolor=\"$greenbdr\" class=\"whitemain\" height=\"23\">&nbsp;CCL Documents Matching \"$search\"</td>\n";
            echo "  </tr>\n";
            echo "  <tr>\n";
            echo "  <td bgcolor=\"#FFFFFF\" valign=\"top\" bordercolor=\"#FFFFFF\" class=\"main\" colspan=\"2\">\n";
        ...
        echo "  <table width=\"100%\" cellspacing=\"0\" cellpadding=\"3\">\n";
        echo "   <tr class=\"trrow1\">\n";
        echo "    <td width=\"100%\"><p>There are no CCL documents matching that query in any way.</p>\n";
        ...
        $historycontains = "article LIKE '%{$search}%' OR title LIKE '%{$search}%'";

        $db->Query("SELECT * FROM history WHERE $historycontains ORDER BY id DESC");
            if ($db->rows)
            {

            echo "<table width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"0\" bordercolor=\"$yellowbdr\" align=\"center\">\n";
            echo "  <tr>\n";
            echo "    <td bgcolor=\"$yellowbdr\" class=\"whitemain\" height=\"23\">&nbsp;History Articles Matching \"$search\"</td>\n";
            echo "  </tr>\n";
            echo "  <tr>\n";
            echo "  <td bgcolor=\"#FFFFFF\" valign=\"top\" bordercolor=\"#FFFFFF\" class=\"main\" colspan=\"2\">\n";
        ...
        echo "  <table width=\"100%\" cellspacing=\"0\" cellpadding=\"3\">\n";
        echo "   <tr class=\"trrow1\">\n";
        echo "    <td width=\"100%\"><p>There are no history articles matching that query in any way.</p>\n";
