...
function show_statistics_listing($db,$statistics,$id,$pr,$team,$week,$game_id)
{
    ...
    if ($db->Exists("SELECT * FROM seasons")) {
        $db->QueryRow("SELECT la.season, se.SeasonName FROM scorecard_batting_details la INNER JOIN seasons se ON la.season = se.SeasonID WHERE se.SeasonName NOT LIKE '%KO%' GROUP BY la.season ORDER BY se.SeasonName DESC");
    ...
    //////////////////////////////////////////////////////////////////////////////////////////      
        ...
        $db->QueryRow("SELECT * FROM teams WHERE TeamActive=1 AND LeagueID=2 ORDER BY TeamName");
            for ($x=0; $x<$db->rows; $x++) {
                $db->GetRow($x);
                $db->BagAndTag();
                $id = $db->data['TeamID'];
...
function show_statistics_team($db,$statistics,$id,$pr,$team,$week,$game_id)
{
    ...
    ON
      g.season = n.SeasonID
    WHERE 
      g.league_id=2 AND p.PlayerTeam = $team AND n.SeasonName LIKE '%{$statistics}%' 
    GROUP BY 
      s.player_id        
        ")) {
    ...
    if (!$subdb->Exists("SELECT COUNT(b.runs) AS Hundred FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.runs >= 100")){
      $schun = "-";
    } else {
    $subdb->QueryRow("SELECT COUNT(b.runs) AS Hundred FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.runs >= 100");
    ...
    if (!$subdb->Exists("SELECT COUNT(b.runs) AS Fifty FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND (b.runs BETWEEN 50 AND 99)")) {
      $scfif = "-";
    } else {
    $subdb->QueryRow("SELECT COUNT(b.runs) AS Fifty FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND (b.runs BETWEEN 50 AND 99)");
    ...
    if (!$subdb->Exists("SELECT COUNT(b.assist) AS Caught FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 4")) {
      $scctc = "0";
    } else {
    $subdb->QueryRow("SELECT COUNT(b.assist) AS Caught FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 4");
      $scctc = $subdb->data[Caught];    
    }

    // Get Caught and Bowleds
    
    if (!$subdb->Exists("SELECT COUNT(b.bowler) AS CandB FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.bowler = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 5")) {
      $sccab = "0";
    } else {
    $subdb->QueryRow("SELECT COUNT(b.bowler) AS CandB FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.bowler = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 5");
    ...
    if (!$subdb->Exists("SELECT COUNT(b.assist) AS Stumped FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 10")) {
      $scstu = "-";
    } else {
    $subdb->QueryRow("SELECT COUNT(b.assist) AS Stumped FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 10");
    ...
    $subdb->QueryRow("SELECT b.notout, MAX(b.runs) AS HS FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' GROUP BY b.notout ORDER BY HS DESC LIMIT 1");
    ...
    $subdb->QueryRow("SELECT COUNT(b.wickets) AS fourwickets FROM scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.wickets = 4");
    ...
    $subdb->QueryRow("SELECT COUNT(b.wickets) AS fivewickets FROM scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.wickets >= 5");
...
function show_statistics_mostruns($db,$statistics,$sort,$sort2,$option,$team)
{
    ...
    if($option == "byseason") {
      if($sort == "Average") echo "<b class=\"16px\">$statistics Batting - Highest Averages</b><br><br>\n";
      if($sort == "Runs") echo "<b class=\"16px\">$statistics Batting - Most Runs</b><br><br>\n";
    }
    
    if($option == "allcareer") {
      if($sort == "Average") echo "<b class=\"16px\">Career Batting - Highest Averages</b><br>From <b>$d</b> to the present.<br><br>\n";
      if($sort == "Runs") echo "<b class=\"16px\">Career Batting - Most Runs</b><br>From <b>$d</b> to the present.<br><br>\n";  
    }
    
    if($option == "teamcareer") {
      if($sort == "Average") echo "<b class=\"16px\">{$teams[$team]} Career Batting - Highest Averages</b><br>From <b>$d</b> to the present.<br><br>\n";
      if($sort == "Runs") echo "<b class=\"16px\">{$teams[$team]} Career Batting - Most Runs</b><br>From <b>$d</b> to the present.<br><br>\n";  
    }
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.runs) AS Hundred FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND b.runs >= 100");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.runs) AS Hundred FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid  AND b.runs >= 100");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.runs) AS Hundred FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid  AND b.team=$team AND b.runs >= 100");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.runs) AS Fifty FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND (b.runs BETWEEN 50 AND 99) ");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.runs) AS Fifty FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid  AND (b.runs BETWEEN 50 AND 99) ");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.runs) AS Fifty FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid  AND b.team=$team AND (b.runs BETWEEN 50 AND 99) ");      
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.assist) AS Caught FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 4");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.assist) AS Caught FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid  AND b.how_out = 4");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.assist) AS Caught FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid  AND b.opponent=$team AND b.how_out = 4");

      $scctc = $subdb->data[Caught];    

    // Get Caught and Bowleds

    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.bowler) AS CandB FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.bowler = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 5");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.bowler) AS CandB FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.bowler = $playerid  AND b.how_out = 5");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.bowler) AS CandB FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.bowler = $playerid  AND b.opponent=$team AND b.how_out = 5");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.assist) AS Stumped FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid  AND s.SeasonName LIKE '%{$statistics}%' AND b.how_out = 10");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.assist) AS Stumped FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid  AND b.how_out = 10");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.assist) AS Stumped FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.assist = $playerid  AND b.opponent=$team AND b.how_out = 10");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT b.notout, MAX(b.runs) AS HS FROM scorecard_batting_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' GROUP BY b.notout ORDER BY HS DESC LIMIT 1");
    if($option == "allcareer")  $subdb->QueryRow("SELECT b.notout, MAX(b.runs) AS HS FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid GROUP BY b.notout ORDER BY HS DESC LIMIT 1");
    if($option == "teamcareer") $subdb->QueryRow("SELECT b.notout, MAX(b.runs) AS HS FROM scorecard_batting_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND b.team=$team GROUP BY b.notout ORDER BY HS DESC LIMIT 1");       
...
function show_statistics_bestinnings($db,$statistics,$option,$team)
{
    ...
    if($option == "byseason")   echo "<b class=\"16px\">$statistics Batting - Highest Innings Scores</b><br>Including Knock-Outs. Qualification 35 runs.<br><br>\n";
...
function show_statistics_bowling($db,$statistics,$sort,$direction,$option,$team)
{
    ...
    if($option == "byseason") {
      if($sort == "Wickets") echo "<b class=\"16px\">$statistics Bowling - Most Wickets</b><br><br>\n";
      if($sort == "Balls")   echo "<b class=\"16px\">$statistics Bowling - Workhorses</b><br><br>\n";
      if($sort == "Average") echo "<b class=\"16px\">$statistics Bowling - Best Averages</b><br><br>\n";
    }

    if($option == "allcareer") {
      if($sort == "Wickets") echo "<b class=\"16px\">Career Bowling - Most Wickets</b><br>From <b>$d</b> to the present.<br><br>\n";
      if($sort == "Balls")   echo "<b class=\"16px\">Career Bowling - Workhorses</b><br>From <b>$d</b> to the present.<br><br>\n";
      if($sort == "Average") echo "<b class=\"16px\">Career Bowling - Best Averages</b><br>From <b>$d</b> to the present.<br><br>\n";
    }
    ...
    //////////////////////////////////////////////////////////////////////////////////////////
        ...
        for ($x=0; $x<$db->rows; $x++) {
            ...
            $sid = $db->data['season'];
              ...
              if($sort == "Wickets") echo "<option value=\"$PHP_SELF?option=byseason&statistics=$sen&sort=Wickets&direction=desc&ccl_mode=5\" class=\"10px\">$sen</option>\n";
              if($sort == "Balls") echo "<option value=\"$PHP_SELF?option=byseason&statistics=$sen&sort=Balls&direction=desc&ccl_mode=5\" class=\"10px\">$sen</option>\n";
        }
    ...
    for ($r=0; $r<$db->rows; $r++) {
      ...
      if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.wickets) AS fourwickets FROM scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.wickets = 4");
      if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.wickets) AS fourwickets FROM scorecard_bowling_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND b.wickets = 4");
      if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.wickets) AS fourwickets FROM scorecard_bowling_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND b.team=$team AND b.wickets = 4");
    ...
    if($option == "byseason")   $subdb->QueryRow("SELECT COUNT(b.wickets) AS fivewickets FROM scorecard_bowling_details b INNER JOIN seasons s ON b.season = s.SeasonID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND s.SeasonName LIKE '%{$statistics}%' AND b.wickets >= 5");
    if($option == "allcareer")  $subdb->QueryRow("SELECT COUNT(b.wickets) AS fivewickets FROM scorecard_bowling_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND b.wickets >= 5");
    if($option == "teamcareer") $subdb->QueryRow("SELECT COUNT(b.wickets) AS fivewickets FROM scorecard_bowling_details b INNER JOIN scorecard_game_details g ON b.game_id = g.game_id WHERE g.league_id=2 AND b.player_id = $playerid AND b.team=$team AND b.wickets >= 5");
...
function show_statistics_bestbowling($db,$statistics,$option,$team)
{
    ...
    if($option == "byseason")   echo "<b class=\"16px\">$statistics Bowling - Best Innings Bowling</b><br>Including Knock-Outs. Qualification 3 wickets.<br><br>\n";
    ...
    if($option == "byseason")   $db->Query("SELECT g.season, n.SeasonName, g.game_id, g.game_date, g.awayteam, g.hometeam, r.GroundAbbrev AS Ground, a.TeamAbbrev AS 'awayabbrev', h.TeamAbbrev AS 'homeabbrev', b.player_id, b.runs AS BRuns, b.wickets AS Wickets, p.PlayerLName, p.PlayerLAbbrev, p.PlayerFName, LEFT(p.PlayerFName,1) AS PlayerInitial FROM scorecard_bowling_details b INNER JOIN players p ON b.player_id = p.PlayerID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id INNER JOIN teams a ON g.awayteam = a.TeamID INNER JOIN teams h ON g.hometeam = h.TeamID INNER JOIN grounds r ON g.ground_id = r.GroundID INNER JOIN seasons n ON g.season = n.SeasonID WHERE g.league_id=2 AND n.SeasonName LIKE '%{$statistics}%' AND Wickets >= 3 ORDER BY Wickets DESC, Runs ASC");
    if($option == "allcareer")  $db->Query("SELECT g.season, g.game_id, g.game_date, g.awayteam, g.hometeam, r.GroundAbbrev AS Ground, a.TeamAbbrev AS 'awayabbrev', h.TeamAbbrev AS 'homeabbrev', b.player_id, b.runs AS BRuns, b.wickets AS Wickets, p.PlayerLName, p.PlayerLAbbrev, p.PlayerFName, LEFT(p.PlayerFName,1) AS PlayerInitial FROM scorecard_bowling_details b INNER JOIN players p ON b.player_id = p.PlayerID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id INNER JOIN teams a ON g.awayteam = a.TeamID INNER JOIN teams h ON g.hometeam = h.TeamID INNER JOIN grounds r ON g.ground_id = r.GroundID WHERE g.league_id=2 AND Wickets >= 4 ORDER BY Wickets DESC, Runs ASC");
    if($option == "teamcareer") $db->Query("SELECT g.season, g.game_id, g.game_date, g.awayteam, g.hometeam, r.GroundAbbrev AS Ground, a.TeamAbbrev AS 'awayabbrev', h.TeamAbbrev AS 'homeabbrev', b.player_id, b.runs AS BRuns, b.wickets AS Wickets, p.PlayerLName, p.PlayerLAbbrev, p.PlayerFName, LEFT(p.PlayerFName,1) AS PlayerInitial FROM scorecard_bowling_details b INNER JOIN players p ON b.player_id = p.PlayerID INNER JOIN scorecard_game_details g ON b.game_id = g.game_id INNER JOIN teams a ON g.awayteam = a.TeamID INNER JOIN teams h ON g.hometeam = h.TeamID INNER JOIN grounds r ON g.ground_id = r.GroundID WHERE g.league_id=2 AND p.PlayerTeam = $team AND Wickets >= 3 ORDER BY Wickets DESC, Runs ASC");
...
function show_statistics_allrounders($db,$statistics,$option,$team)
{
    ...
    if($option == "byseason")   echo "<b class=\"16px\">$statistics All-Round - 100 Runs & 10 Wickets</b><br>Including Knock-Outs<br><br>\n";
    ...
    if($option == "byseason")   $db->Query("SELECT g.season, n.SeasonName, t.TeamID, t.TeamAbbrev, COUNT( s.player_id ) AS Matches, SUM( s.runs ) AS Runs, MAX( s.runs ) AS HS, s.player_id, p.PlayerID, LEFT(p.PlayerFName,1) AS PlayerInitial, p.PlayerFName, p.PlayerLAbbrev, p.PlayerLName FROM scorecard_batting_details s INNER JOIN players p ON s.player_id = p.PlayerID INNER JOIN scorecard_game_details g ON s.game_id = g.game_id INNER JOIN teams t ON p.PlayerTeam = t.TeamID INNER JOIN seasons n ON g.season = n.SeasonID WHERE g.league_id=2 AND n.SeasonName LIKE '%{$statistics}%' GROUP BY p.PlayerID ORDER BY p.PlayerLName");
    if($option == "allcareer")  $db->Query("SELECT t.TeamID, t.TeamAbbrev, COUNT( s.player_id ) AS Matches, SUM( s.runs ) AS Runs, MAX( s.runs ) AS HS, s.player_id, p.PlayerID, LEFT(p.PlayerFName,1) AS PlayerInitial, p.PlayerFName, p.PlayerLAbbrev, p.PlayerLName FROM scorecard_batting_details s INNER JOIN players p ON s.player_id = p.PlayerID INNER JOIN scorecard_game_details g ON s.game_id = g.game_id INNER JOIN teams t ON p.PlayerTeam = t.TeamID WHERE g.league_id=2 AND GROUP BY p.PlayerID ORDER BY p.PlayerLName");
    if($option == "teamcareer") $db->Query("SELECT t.TeamID, t.TeamAbbrev, COUNT( s.player_id ) AS Matches, SUM( s.runs ) AS Runs, MAX( s.runs ) AS HS, s.player_id, p.PlayerID, LEFT(p.PlayerFName,1) AS PlayerInitial, p.PlayerFName, p.PlayerLAbbrev, p.PlayerLName FROM scorecard_batting_details s INNER JOIN players p ON s.player_id = p.PlayerID INNER JOIN scorecard_game_details g ON s.game_id = g.game_id INNER JOIN teams t ON p.PlayerTeam = t.TeamID WHERE g.league_id=2 AND p.PlayerTeam = $team GROUP BY p.PlayerID ORDER BY p.PlayerLName");
