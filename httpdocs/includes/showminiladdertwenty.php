		...
		")) {
			...
			$db->Query("
				...
				INNER JOIN
                  groups gp ON tm.TeamID = gp.TeamID
                WHERE
                  lad.season = gp.SeasonID AND season=$sid AND gp.GroupName = '$grpnm' AND Round = 1 ORDER BY lad.rank_sort ASC
			");


			for ($x=0; ($x<$db->rows && $x<4); $x++) {
				$db->GetRow($x);
			...
			if($t == $subdb1->rows - 1) {
				echo "<tr class=\"trrow1\">\n";
				echo "	<td align=\"center\" colspan=8 style=\"border: solid #DE9C06; border-top: none;\"><a href=\"/ladder.php?ladder=$sid&round=1&ccl_mode=1\" class=\"right\">More Details</a></td>\n";
				echo "</tr>\n";
			}
            echo "</table>\n";
			}
