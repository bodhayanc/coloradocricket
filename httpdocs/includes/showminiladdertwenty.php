		...
		")) {
            ...
            $db->Query("SELECT hometeam as team FROM schedule s, groups g WHERE s.season=$sid AND g.SeasonID = s.season AND s.hometeam = g.TeamID AND g.GroupName = '$grpnm' AND g.Round = 2
                ...
                for ($x=0; $x<$db->rows; $x++) {
					...
					$subdb->QueryRow("SELECT IFNULL(SUM(CASE WHEN (t.dl_total IS NOT NULL AND t.dl_total != 0) THEN t.dl_total ELSE t.total END), 0) AS TotalRunsFor FROM scorecard_game_details g INNER JOIN scorecard_total_details t ON t.game_id = g.game_id WHERE (g.cancelled = 0 and g.cancelledplay = 0) and g.season = $sid and  ((g.hometeam = $tid and t.team = g.hometeam and g.awayteam in ($groupTeams)) OR (g.awayteam = $tid and t.team = g.awayteam and g.hometeam in ($groupTeams)))");
					$subdb->BagAndTag();
					$trf = $subdb->data['TotalRunsFor'];
					$subdb->QueryRow("SELECT IFNULL(SUM(CASE WHEN (t.dl_total IS NOT NULL AND t.dl_total != 0) THEN t.dl_total ELSE t.total END), 0) AS TotalRunsAgainst FROM scorecard_game_details g INNER JOIN scorecard_total_details t ON t.game_id = g.game_id WHERE (g.cancelled = 0 and g.cancelledplay = 0) and g.season = $sid and ((g.hometeam = $tid and t.team = g.awayteam and g.awayteam in ($groupTeams)) OR (g.awayteam = $tid and t.team = g.hometeam and g.hometeam in ($groupTeams)))");
					$subdb->BagAndTag();
					$tra = $subdb->data['TotalRunsAgainst'];
					$subdb->QueryRow("SELECT (IFNULL(TotalOversForFirst, 0) + IFNULL(TotalOversForSecondWin, 0) + IFNULL(TotalOversForSecondLoose, 0)) AS TotalOverFor FROM (SELECT TotalOversConvForFirst + TotalBallsForFirst AS TotalOversForFirst, TotalOversConvForSecondWin + TotalBallsForSecondWin AS TotalOversForSecondWin, TotalOversConvForSecondLoose + TotalBallsForSecondLoose AS TotalOversForSecondLoose FROM (SELECT sum(SUBSTRING_INDEX(FORMAT(g.maxovers, 1),'.',1)) * 6 AS TotalOversConvForFirst, sum(SUBSTRING_INDEX(FORMAT(g.maxovers, 1),'.',-1)) AS TotalBallsForFirst FROM scorecard_game_details g INNER JOIN scorecard_total_details t ON t.game_id = g.game_id WHERE (g.cancelled = 0 and g.cancelledplay = 0) and g.season = $sid and g.batting_first_id = $tid and g.batting_second_id in ($groupTeams) and t.innings_id = 1) sums,(SELECT sum(SUBSTRING_INDEX(t.overs,'.',1)) * 6 AS TotalOversConvForSecondWin, sum(SUBSTRING_INDEX(FORMAT(t.overs, 1),'.',-1)) AS TotalBallsForSecondWin FROM scorecard_game_details g INNER JOIN scorecard_total_details t ON t.game_id = g.game_id WHERE (g.cancelled = 0 and g.cancelledplay = 0) and g.season = $sid and g.batting_second_id = $tid and g.batting_first_id in ($groupTeams) and t.innings_id = 2 and g.result_won_id = $tid) sumsForSecWin,(SELECT sum(SUBSTRING_INDEX(g.maxovers,'.',1)) * 6 AS TotalOversConvForSecondLoose, sum(SUBSTRING_INDEX(FORMAT(g.maxovers, 1),'.',-1)) AS TotalBallsForSecondLoose FROM scorecard_game_details g INNER JOIN scorecard_total_details t ON t.game_id = g.game_id WHERE (g.cancelled = 0 and g.cancelledplay = 0) and g.season = $sid and g.batting_second_id = $tid and g.batting_first_id in ($groupTeams) and t.innings_id = 2 and g.result_won_id != $tid) sumForSecLoose) sumOversFor");
					$subdb->BagAndTag();
					$tof = $subdb->data['TotalOverFor'];
					$subdb->QueryRow("SELECT (IFNULL(TotalOversAgainstFirst, 0) + IFNULL(TotalOversAgainstSecondWin, 0) + IFNULL(TotalOversAgainstSecondLoose, 0)) AS TotalOverAgainst FROM (SELECT TotalOversConvAgainstFirst + TotalBallsAgainstFirst AS TotalOversAgainstFirst, TotalOversConvAgainstSecondWin + TotalBallsAgainstSecondWin AS TotalOversAgainstSecondWin, TotalOversConvAgainstSecondLoose + TotalBallsAgainstSecondLoose AS TotalOversAgainstSecondLoose FROM (SELECT sum(SUBSTRING_INDEX(FORMAT(g.maxovers, 1),'.',1)) * 6 AS TotalOversConvAgainstFirst, sum(SUBSTRING_INDEX(FORMAT(g.maxovers, 1),'.',-1)) AS TotalBallsAgainstFirst FROM scorecard_game_details g INNER JOIN scorecard_total_details t ON t.game_id = g.game_id WHERE (g.cancelled = 0 and g.cancelledplay = 0) and g.season = $sid and g.batting_second_id = $tid and g.batting_first_id in ($groupTeams) and t.innings_id = 1) sumAgainstFirst,(SELECT sum(SUBSTRING_INDEX(g.maxovers,'.',1)) * 6 AS TotalOversConvAgainstSecondWin, sum(SUBSTRING_INDEX(FORMAT(g.maxovers, 1),'.',-1)) AS TotalBallsAgainstSecondWin FROM scorecard_game_details g INNER JOIN scorecard_total_details t ON t.game_id = g.game_id WHERE (g.cancelled = 0 and g.cancelledplay = 0) and g.season = $sid and g.batting_first_id = $tid and g.batting_second_id in ($groupTeams) and t.innings_id = 2 and g.result_won_id = $tid) sumsAgainstSecWin,(SELECT sum(SUBSTRING_INDEX(FORMAT(t.overs, 1),'.',1)) * 6 AS TotalOversConvAgainstSecondLoose, sum(SUBSTRING_INDEX(FORMAT(t.overs, 1),'.',-1)) AS TotalBallsAgainstSecondLoose FROM scorecard_game_details g INNER JOIN scorecard_total_details t ON t.game_id = g.game_id WHERE (g.cancelled = 0 and g.cancelledplay = 0) and g.season = $sid and g.batting_first_id = $tid and g.batting_second_id in ($groupTeams) and t.innings_id = 2 and g.result_won_id != $tid) sumAgainstSecLoose) sumOversAgainst");
					$subdb->BagAndTag();
					$toa = $subdb->data['TotalOverAgainst'];
					if($trf > 0 && $tof > 0) {
						if($tra > 0 && $toa > 0) {
					...
					$pl = $subdb->QueryItem("SELECT count(game_id) FROM
						   scorecard_game_details WHERE season=$sid AND ((hometeam=$tid and awayteam in ($groupTeams)) OR (awayteam=$tid and hometeam in ($groupTeams))) AND (points = 1 OR points = \"\" OR points is NULL)");
					$wo = $subdb->QueryItem("SELECT count(game_id) FROM
						   scorecard_game_details WHERE season=$sid AND ((hometeam=$tid and awayteam in ($groupTeams)) OR (awayteam=$tid and hometeam in ($groupTeams))) AND cancelledplay=0 AND cancelled=0 AND result_won_id = $tid");
					$ti = $subdb->QueryItem("SELECT count(game_id) FROM
						   scorecard_game_details WHERE season=$sid AND ((hometeam=$tid and awayteam in ($groupTeams)) OR (awayteam=$tid and hometeam in ($groupTeams))) AND cancelledplay=0 AND cancelled=0 AND result_won_id=0");
					$nr = $subdb->QueryItem("SELECT count(game_id) FROM
						   scorecard_game_details WHERE season=$sid AND ((hometeam=$tid and awayteam in ($groupTeams)) OR (awayteam=$tid and hometeam in ($groupTeams))) AND (cancelledplay=1 OR cancelled=1) AND points = 1");
					$lo = $pl - $wo - $ti - $nr;
					$pt = ($wo * 2) + $ti + $nr;
					$tp = $pt - $pe;
					$tof_o = Round(($tof / 6), 2); 
