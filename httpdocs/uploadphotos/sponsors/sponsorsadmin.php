...
function do_update_category($db,$id,$title,$url,$promised,$article,$setpic)
{
	global $content,$action,$SID,$bluebdr,$greenbdr,$yellowbdr;
...
// prevent the need for using escape sequences with apostrophe's
	...
	$db->Update("UPDATE sponsors SET title='$t',url='$u',promised='$p',article='$a'$setpic WHERE id=$id");
...
if ($_FILES['userpic1']['name'] != "") {
  $uploaddir1 = "../uploadphotos/sponsors/";
...
case "sedit":
	...
	else do_update_category($db,$id,$title,$url,$promised,$article,$setpic);
	break;
default:
	show_main_menu($db);
	break;
