...
function show_main_menu($db)
{
    ...
    if (!$db->Exists("SELECT * FROM tennisteams")) {
      ...
      echo "    <td>\n";
        ...
        $db->Query("SELECT t.*, g.* FROM tennisteams t INNER JOIN tennisgroups g ON t.TeamGroup = g.GroupID ORDER BY g.GroupName ASC, t.TeamName ASC");
        for ($x=0; $x<$db->rows; $x++) {
            $db->GetRow($x);
...
function add_category_form($db)
{
    ...
    */
    ...
    echo "<p>select group this team belongs to<br><select name=\"TeamGroup\">\n";
    ...
    if ($db->Exists("SELECT * FROM tennisgroups")) {
        ...
        for ($i=0; $i<$db->rows; $i++) {
            $db->GetRow($i);
            echo "<option value=\"" . $db->data[GroupID] . "\">" . $db->data[GroupName] . "</option>\n";
        }
    }
...
function do_add_category($db,$ClubID,$TeamName,$TeamAbbrev,$TeamGroup,$TeamURL,$TeamColour,$TeamActive,$TeamDesc,$picture)
{
    global $content,$action,$SID,$bluebdr,$greenbdr,$yellowbdr;
    ...
    if ($db->Exists("SELECT * FROM tennisteams WHERE TeamName='$tn'")) {
    ...
    $db->Insert("INSERT INTO tennisteams (ClubID,TeamName,TeamAbbrev,TeamGroup,TeamURL,TeamColour,TeamActive,TeamDesc,picture) VALUES ('$cl','$tn','$ta','$tg','$ur','$tc','$tv','$td','$picture')");
    if ($db->a_rows != -1) {
...
function delete_category_check($db,$id)
{
    ...
    $team = htmlentities(stripslashes($db->QueryItem("SELECT TeamName FROM tennisteams WHERE TeamID=$id")));
...
function do_delete_category($db,$id,$doit)
{
    ...
    // do delete
        ...
        $db->Delete("DELETE FROM tennisteams WHERE TeamID=$id");
...
function edit_category_form($db,$id)
{
    ...
    $db->QueryRow("SELECT * FROM tennisteams WHERE TeamID=$id");
    ...
    echo "<p>select group this team belongs to<br>\n";
    echo "<select name=\"TeamGroup\">\n";
    echo "  <option value=\"\">Select Group</option>\n";
    echo "  <option value=\"\">--------------------------</option>\n";

    foreach ($groups as $id => $name) {
        echo '<option value="', $id, '"', ($id == $db->data['TeamGroup'] ? ' selected' : ''), ">$name</option>\n";
    }
...
function do_update_category($db,$id,$ClubID,$TeamName,$TeamAbbrev,$TeamGroup,$TeamURL,$TeamColour,$TeamActive,$TeamDesc,$setpic)
{
    global $content,$action,$SID,$bluebdr,$greenbdr,$yellowbdr;
...
// query database
    ...
    $db->Update("UPDATE tennisteams SET ClubID='$cl',TeamName='$tn',TeamAbbrev='$ta',TeamGroup='$tg',TeamURL='$ur',TeamColour='$tc',TeamActive='$tv',TeamDesc='$td'$setpic WHERE TeamID=$id");
...
// put picture in right place
    ...
    if (!copy($userpic,"../uploadphotos/teams/$picture")) {
...
case "sadd":
    ...
    else do_add_category($db,$ClubID,$TeamName,$TeamAbbrev,$TeamGroup,$TeamURL,$TeamColour,$TeamActive,$TeamDesc,$picture);
    break;
case "sdel":
...
case "sedit":
    ...
    else do_update_category($db,$id,$ClubID,$TeamName,$TeamAbbrev,$TeamGroup,$TeamURL,$TeamColour,$TeamActive,$TeamDesc,$setpic);
    break;
default:
    show_main_menu($db);
    break;
