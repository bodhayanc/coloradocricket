...
function show_main_menu($db)
{
	...
	if (!$db->Exists("SELECT * FROM grounds")) {
	  ...
	  echo "    <td>\n";
		...
		$db->Query("SELECT * FROM grounds WHERE LeagueID = 2 ORDER BY GroundName");
		for ($x=0; $x<$db->rows; $x++) {
			$db->GetRow($x);
...
// setup the variables
	...
	$db->Update("UPDATE grounds SET LeagueID=2,GroundName='$gn',GroundAbbrev='$gb',GroundLoc='$gl',GroundDirections='$gd',GroundZip='$gz',description='$de',parking='$pa',coveredparking='$cp',shelter='$sh',handicapped='$ha',stadiumseating='$ss',restrooms='$rr',conveniencestore='$cs',drinkingwater='$dw',publictransport='$pt',GroundActive='$ga'$setpic WHERE GroundID=$id");
...
echo "<p class=\"16px\"><b>Cougars Grounds Administration</b></p>\n";
