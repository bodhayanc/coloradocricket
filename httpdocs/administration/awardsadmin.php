...
function show_main_menu($db)
{
    ...
    // 23-Jan-2014 12:23am Added DESC; 20-Nov-017 8:47pm Added NOT LIKE '%T16%' OR '%CTCL%'
	// if (!$db->Exists("SELECT * FROM seasons WHERE SeasonName NOT LIKE '%KO%' ORDER BY SeasonName")) {
        if (!$db->Exists("SELECT * FROM seasons WHERE NOT (SeasonName LIKE '%T16%' OR SeasonName LIKE '%CTCL%') ORDER BY SeasonName DESC")) {
	  ...
	  echo "    <td>\n";
		...
		// $db->Query("SELECT * FROM seasons WHERE SeasonName NOT LIKE '%KO%' ORDER BY SeasonName");
               $db->Query("SELECT * FROM seasons WHERE NOT (SeasonName LIKE '%T16%' OR SeasonName LIKE '%CTCL%') ORDER BY SeasonName DESC");  // 23-Jan-2014 12:23am Added DESC
		for ($x=0; $x<$db->rows; $x++) {
			$db->GetRow($x);
...
function show_main_menu_season($db,$season,$sename)
{
	...
	// check for empty database
    // 23-Jan-2014 12:13am Changed the ORDER BY from fm.AwardID DESC to awt.rank_sort
	if (!$db->Exists("
			SELECT
			...
			INNER JOIN
				awardtypes awt ON fm.AwardTitle = awt.AwardID
			INNER JOIN
...
function add_category_form($db)
{
	...
	if ($db->Exists("SELECT * FROM players where IsActive=0 ")) {
		$db->Query("SELECT * FROM players where IsActive=0 ORDER BY PlayerLName, PlayerFName"); // 23-Jan-2015 12:15am Added PlayerFName
		for ($i=0; $i<$db->rows; $i++) {
			$db->GetRow($i);
...
function edit_category_form($db,$id)
{
	...
	// get all players
	$db->Query("SELECT PlayerID, PlayerLName, CONCAT(PlayerLName,', ',PlayerFName) AS PlayerName FROM players ORDER BY PlayerLName, PlayerFName"); // 23-Jan-2015 12:15am Added PlayerFName
	for ($p=0; $p<$db->rows; $p++) {
		$db->GetRow($p);
        $db->BagAndTag();
