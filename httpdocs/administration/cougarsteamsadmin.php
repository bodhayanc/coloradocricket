...
function do_add_category($db,$ClubID,$TeamName,$TeamAbbrev,$TeamURL,$TeamColour,$TeamActive,$TeamDesc,$picture)
{
	...
	$db->Insert("INSERT INTO teams (ClubID,LeagueID,TeamName,TeamAbbrev,TeamURL,TeamColour,TeamActive,TeamDesc,picture) VALUES ('$cl',2,'$tn','$ta','$ur','$tc','$tv','$td','$picture')");
	if ($db->a_rows != -1) {
...
function edit_category_form($db,$id)
{
	...
	$db->QueryRow("SELECT * FROM teams WHERE LeagueID = 2 AND TeamID=$id");
...
// query database
	...
	$db->Update("UPDATE teams SET ClubID='$cl',LeagueID=2,TeamName='$tn',TeamAbbrev='$ta',TeamURL='$ur',TeamColour='$tc',TeamActive='$tv',TeamDesc='$td'$setpic WHERE TeamID=$id");
