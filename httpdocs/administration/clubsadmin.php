...
function show_main_menu($db)
{
	...
	if (!$db->Exists("SELECT * FROM clubs")) {
	  ...
	  echo "    <td>\n";
		...
		$db->Query("SELECT * FROM clubs WHERE LeagueID = 1 ORDER BY ClubActive DESC, ClubName");
		for ($x=0; $x<$db->rows; $x++) {
			$db->GetRow($x);
...
function do_add_category($db,$ClubName,$ClubURL,$ClubColour,$ClubActive,$GroundID)
{
	...
	if ($db->Exists("SELECT * FROM clubs WHERE LeagueID = 1 AND ClubName='$cn'")) {
	...
	$db->Insert("INSERT INTO clubs (ClubName,ClubURL,ClubColour,ClubActive,GroundID,LeagueID) VALUES ('$cn','$ur','$cc','$ca','$gr',1)");
	if ($db->a_rows != -1) {
...
// setup the variables
	...
	$db->Update("UPDATE clubs SET ClubName='$cn',ClubURL='$ur',ClubColour='$cc',ClubActive='$ca',GroundID='$gr', LeagueID=1 WHERE ClubID=$id");
