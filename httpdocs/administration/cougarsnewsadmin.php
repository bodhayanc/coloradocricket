...
function show_recent_main_menu($db)
{
	...
	if (!$db->Exists("SELECT * FROM news WHERE IsFeature != 0 AND newstype = 2")) {
	...
	echo "</table><br>\n";
	  ...
	  echo "    <td>\n";
		...
		$db->Query("SELECT * FROM news WHERE IsFeature != 1 AND newstype = 2 ORDER BY id DESC LIMIT 10");
		for ($x=0; $x<$db->rows; $x++) {
			$db->GetRow($x);
...
//			echo "	<td align=\"right\"><a href=\"main.php?SID=$SID&action=$action&do=sedit&id=" . $db->data[id] . "\"><img src=\"/images/icons/icon_edit.gif\" border=\"0\" alt=\"Edit\"></a><a 
// href=\"main.php?SID=$SID&action=$action&do=sdel&id=" . $db->data[id] . "\"><img src=\"/images/icons/icon_delete.gif\" border=\"0\" alt=\"Delete\"></a></td>\n";
...
function show_months_main_menu($db,$theyear,$themonth,$monthname)
{
	...
	echo "</table><br>\n";
	  ...
	  echo "    <td>\n";
		...
		$db->Query("SELECT * FROM news WHERE IsFeature != 1 AND newstype = 2 AND year(added) = $theyear AND month(added) = $themonth ORDER BY id DESC");
		for ($x=0; $x<$db->rows; $x++) {
			$db->GetRow($x);
...
function show_search_main_menu($db,$search="",$theyear,$themonth,$monthname)
{
	...
	if ($search != "")
	{
		...
		$db->Query("SELECT * FROM news WHERE $contains AND newstype = 2 ORDER BY id DESC");
			if ($db->rows)
			{
...
function show_main_menu($db)
{
	...
	echo "</table><br>\n";
	  ...
	  echo "    <td>\n";
		...
		$db->Query("SELECT * FROM news WHERE IsFeature != 1 AND newstype = 2 ORDER BY id DESC");
		for ($x=0; $x<$db->rows; $x++) {
			$db->GetRow($x);
...
function add_category_form($db)
{
	...
	echo "<p>enter the press release<br><textarea name=\"article\" cols=\"70\" rows=\"15\" wrap=\"virtual\"></textarea></p>\n";
	...
	echo "<p>enter a short description of the photo <i>(255 chars max)</i><br><textarea name=\"picdesc\" cols=\"70\" rows=\"15\" wrap=\"virtual\"></textarea></p>\n";	
...
function do_add_category($db,$uid,$title,$author,$article,$IsFeature,$DiscussID,$picdesc,$picture)
{
	...
	$db->Insert("INSERT INTO news (added,user,title,author,article,IsFeature,DiscussID,picdesc,picture,newstype) VALUES ('$d','$uid','$t','$au','$a','$if','$di','$pd','$picture',2)");
	if ($db->a_rows != -1) {
...
//	}
	...
	echo "<p>enter a short description of the photo <i>(255 chars max)</i><br><textarea name=\"picdesc\" cols=\"70\" rows=\"15\" wrap=\"virtual\">$pd</textarea></p>\n";	
...
function do_update_category($db,$id,$title,$author,$article,$IsFeature,$DiscussID,$picdesc,$setpic)
{
	global $content,$action,$SID,$bluebdr,$greenbdr,$yellowbdr;
...
// prevent the need for using escape sequences with apostrophe's
	...
	$db->Update("UPDATE news SET newstype=2,title='$t',author='$au',article='$a',IsFeature='$if',DiscussID='$di',picdesc='$pd'$setpic WHERE id=$id");
...
echo "<p class=\"16px\"><b>Cougars News Administration</b></p>\n";
...
case "sedit":
	...
	else do_update_category($db,$id,$title,$author,$article,$IsFeature,$DiscussID,$picdesc,$setpic);
	break;
default:
