...
function show_main_menu($db)
{
	...
	if (!$db->Exists("SELECT * FROM tennisdocuments")) {
	  ...
	  echo "    <td>\n";
		...
		$db->Query("SELECT * FROM tennisdocuments ORDER BY id DESC");
		for ($x=0; $x<$db->rows; $x++) {
			$db->GetRow($x);
...
function do_add_category($db,$title,$article,$picture)
{
	...
	if ($db->Exists("SELECT * FROM tennisdocuments WHERE title='$t'")) {
	...
	$db->Insert("INSERT INTO tennisdocuments (title,article,picture) VALUES ('$t','$a','$picture')");
	if ($db->a_rows != -1) {
...
function delete_category_check($db,$id)
{
	...
	$title = htmlentities(stripslashes($db->QueryItem("SELECT title FROM tennisdocuments WHERE id=$id")));
...
function do_delete_category($db,$id,$doit)
{
	...
	// do delete
		...
		$db->Delete("DELETE FROM tennisdocuments WHERE id=$id");
...
function edit_category_form($db,$id)
{
	...
	$db->QueryRow("SELECT * FROM tennisdocuments WHERE id=$id");
...
// prevent the need for using escape sequences with apostrophe's
	...
	$db->Update("UPDATE tennisdocuments SET title='$t',article='$a'$setpic WHERE id=$id");
...
echo "<p class=\"16px\"><b>Tennis Documents Administration</b></p>\n";
