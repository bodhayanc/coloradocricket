	...
	if (!$db->Exists("SELECT * FROM cclofficers")) {
	  ...
	  echo "    <td>\n";
		...
		for ($x=0; $x<$db->rows; $x++) {
			...
			echo "	<td align=\"right\"><a href=\"main.php?SID=$SID&action=$action&do=sedit&id=$id\"><img src=\"/images/icons/icon_edit.gif\" border=\"0\" alt=\"Edit\"></a></td>\n";
			echo "</tr>\n";
		}
		echo "</table>\n";
...
function add_category_form($db)
{
	...
	echo "<p>select a player for the $ot position</p>\n";
...
//	echo "<p>enter the ccl officer Title</p><textarea name=\"cclofficerTitle\" cols=\"70\" rows=\"15\" wrap=\"virtual\"></textarea></p>\n";
       ...
       echo "<p>Season Year: <input type=\"text\" name=\"season_year\" size=\"4\" value=\"$sy\"></p>\n";
        echo "<p>Start Date: <input type=\"text\" name=\"start_date\" size=\"10\" value=\"$sdt\" ></p>\n";
        echo "<p>End Date: <input type=\"text\" name=\"end_date\" size=\"10\" value=\"$edt\"></p>\n";
...
function do_add_category($db,$cclofficerPlayerID,$cclofficerTitle,$cclofficerDetail,$cclofficerViews, $season_year, $start_date, $end_date, $status)
{
	...
	$db->Insert("INSERT INTO cclofficers (cclofficerPlayerID,cclofficerTitle,cclofficerDetail,cclofficerViews,season_year, start_date, end_date, status) VALUES ('$fp','$fd','$de','$vi', '$sy', '$std', '$edt', '$sta')");
	if ($db->a_rows != -1) {
...
function do_update_category($db,$id,$cclofficerPlayerID,$cclofficerDetail,$cclofficerViews)
{
	global $content,$action,$SID,$bluebdr,$greenbdr,$yellowbdr;
...
// setup the variables
	...
	$ot = addslashes(trim($cclofficerTitle));
...
// query database
	...
	$db->Update("UPDATE cclofficers SET cclofficerPlayerID='$otid',cclofficerDetail='$det',cclofficerViews='$vie', season_year='$sy', start_date='$std', end_date='$edt', status='$sta' WHERE cclofficerID=$id");
...
case "sadd":
	...
	else do_add_category($db,$cclofficerPlayerID,$cclofficerTitle,$cclofficerDetail,$cclofficerViews);
	break;
case "sdel":
...
case "sedit":
	...
	else do_update_category($db,$id,$cclofficerPlayerID,$cclofficerDetail,$cclofficerViews);
	break;
default:
	show_main_menu($db);
	break;
