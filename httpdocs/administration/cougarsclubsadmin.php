...
function do_add_category($db,$ClubName,$ClubURL,$ClubColour,$ClubActive,$GroundID)
{
	...
	if ($db->Exists("SELECT * FROM clubs WHERE LeagueID = 2 AND ClubName='$cn'")) {
	...
	$db->Insert("INSERT INTO clubs (ClubName,ClubURL,ClubColour,ClubActive,GroundID,LeagueID) VALUES ('$cn','$ur','$cc','$ca','$gr',2)");
	if ($db->a_rows != -1) {
...
// setup the variables
	...
	$db->Update("UPDATE clubs SET ClubName='$cn',ClubURL='$ur',ClubColour='$cc',ClubActive='$ca',GroundID='$gr', LeagueID=2 WHERE ClubID=$id");
...
echo "<p class=\"16px\"><b>Cougars Clubs Administration</b></p>\n";
