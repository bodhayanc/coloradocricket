...
function show_recent_main_menu($db)
{
    ...
    echo "</table><br>\n";
      ...
      echo "    <td>\n";
        ...
        $db->Query("SELECT * FROM news WHERE IsFeature != 1 AND newstype = 3 ORDER BY id DESC LIMIT 10");
        for ($x=0; $x<$db->rows; $x++) {
            $db->GetRow($x);
...
function show_months_main_menu($db,$theyear,$themonth,$monthname)
{
    ...
    if (!$db->Exists("SELECT * FROM news WHERE IsFeature != 0 AND newstype = 3")) {
        echo "<p>There are currently no news articles in the database.</p>\n";
        return;
    } else {
    ...
    echo "</table><br>\n";
      ...
      echo "    <td>\n";
        ...
        $db->Query("SELECT * FROM news WHERE IsFeature != 1 AND newstype = 3 AND year(added) = $theyear AND month(added) = $themonth ORDER BY id DESC");
        for ($x=0; $x<$db->rows; $x++) {
            $db->GetRow($x);
...
function show_main_menu($db)
{
    ...
    echo "</table><br>\n";
      ...
      echo "    <td>\n";
        ...
        $db->Query("SELECT * FROM news WHERE IsFeature != 1 AND newstype = 3 ORDER BY id DESC");
        for ($x=0; $x<$db->rows; $x++) {
            $db->GetRow($x);
...
function do_add_category($db,$uid,$title,$author,$article,$IsFeature,$DiscussID,$picdesc,$picture)
{
    ...
    $db->Insert("INSERT INTO news (added,user,title,author,article,IsFeature,DiscussID,picdesc,picture,newstype) VALUES ('$d','$uid','$t','$au','$a','$if','$di','$pd','$picture',3)");
    if ($db->a_rows != -1) {
...
// prevent the need for using escape sequences with apostrophe's
    ...
    $db->Update("UPDATE news SET newstype=3,title='$t',author='$au',article='$a',IsFeature='$if',DiscussID='$di',picdesc='$pd'$setpic WHERE id=$id");
...
echo "<p class=\"16px\"><b>Tennis News Administration</b></p>\n";
