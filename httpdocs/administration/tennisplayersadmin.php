...
function show_main_menu($db)
{
	...
	$db->QueryRow("SELECT * FROM tennisteams ORDER BY teamactive DESC, TeamName");
...
//               echo "    <td width=\"100%\"><a href=\"main.php?SID=$SID&action=$action&do=byteam&team=$id&teamname=$na\"><b>$na $tactive</b></a>&nbsp;\n";

                If ($isactive ==1) {
		echo "    <td width=\"100%\"><a href=\"main.php?SID=$SID&action=$action&do=byteam&team=$id&teamname=$na\"><b>$na $tactive</b></a>&nbsp;\n";
                   } else {
 		echo "    <td width=\"100%\"><a href=\"main.php?SID=$SID&action=$action&do=byteam&team=$id&teamname=$na\"><b>$na<font color=\"red\">$tactive</font></b></a>&nbsp;\n";
                  }
...
//		echo "	<td align=\"right\"><a href=\"main.php?SID=$SID&action=$action&do=sedit&id=" . $db->data[PlayerID] . "\"><img src=\"/images/icons/icon_edit.gif\" border=\"0\" alt=\"Edit\"></a><a href=\"main.php?SID=$SID&action=$action&do=sdel&id=" . $db->data[PlayerID] . "\"><img src=\"/images/icons/icon_delete.gif\" border=\"0\" alt=\"Delete\"></a></td>\n";
		...
		echo "	<td align=\"right\"><a href=\"main.php?SID=$SID&action=$action&do=sedit&id=" . $db->data[PlayerID] . "\"><img src=\"/images/icons/icon_edit.gif\" border=\"0\" alt=\"Edit\"></a></td>\n";
		echo "  </tr>\n";
	...
	for ($i=0; $i<$db->rows; $i++) {
		...
		echo "    <td width=\"100%\"><a href=\"main.php?SID=$SID&action=$action&do=byteam&team=$id\"><b>$na</b></a>&nbsp;\n";
		echo "    </td>\n";
		echo "  </tr>\n";
	}
	...
	for ($i=0; $i<$db->rows; $i++) {
		...
		echo "    <td width=\"100%\"><a href=\"main.php?SID=$SID&action=$action&do=byteam&team=$id&teamname=$na\"><b>$na</b></a>&nbsp;\n";
		echo "    </td>\n";
		echo "  </tr>\n";
	}
...
function show_byletter_menu($db,$letter)
{
	...
	if ($db->Exists("SELECT te.TeamID, te.TeamAbbrev, pl.* FROM tennisplayers pl INNER JOIN teams te ON pl.PlayerTeam = te.TeamID WHERE pl.PlayerLName LIKE '{$letter}%' ORDER BY pl.PlayerLName")) {
	$db->QueryRow("SELECT te.TeamID, te.TeamAbbrev, pl.* FROM tennisplayers pl INNER JOIN teams te ON pl.PlayerTeam = te.TeamID WHERE pl.PlayerLName LIKE '{$letter}%' ORDER BY pl.PlayerLName");
	$db->BagAndTag();
	for ($r=0; $r<$db->rows; $r++) {
		$db->GetRow($r);
		$id = htmlentities(stripslashes($db->data[PlayerID]));
...
function show_byteam_menu($db,$team,$teamname)
{
	...
	echo "</table><br>\n";
		...
		if (!$db->Exists("SELECT * FROM tennisplayers where PlayerTeam=$team")) {
		...
		$db->Query("SELECT * FROM tennisplayers WHERE PlayerTeam=$team ORDER BY PlayerLName");
		for ($x=0; $x<$db->rows; $x++) {
			$db->GetRow($x);
...
function delete_category_check($db,$id)
{
	...
	$player = htmlentities(stripslashes($db->QueryItem("SELECT PlayerLName FROM tennisplayers WHERE PlayerID=$id")));
...
function do_delete_category($db,$id,$doit)
{
	...
	// do delete
		...
		$db->Delete("DELETE FROM tennisplayers WHERE PlayerID=$id");
...
function edit_category_form($db,$id)
{
	...
	// get all teams
//	$db->Query("SELECT * FROM tennisteams WHERE TeamActive = 1 ORDER BY TeamName ASC");  // 30-June-2014 1:21am
	...
	$db->QueryRow("SELECT * FROM tennisplayers WHERE PlayerID=$id");
...
// put picture in right place
	...
	if (!copy($userpic,"../uploadphotos/players/$picture")) {
...
// put picture1 in right place

	if (!copy($userpic1,"../uploadphotos/players/action/$picture1")) {
		echo "<p>That photo could not be uploaded at this time - no photo was added to the database.</p>\n";
		unlink($userpic1);
		return;
	}
...
echo "<p class=\"16px\"><b>Tennis Player Administration</b></p>\n";
